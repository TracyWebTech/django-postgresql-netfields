...
from netfields.lookups import (NetContained, NetContains, NetContainedOrEqual,
...
class NetfieldsConfig(AppConfig):
    ...
    for lookup in default_lookups.keys():
        if lookup not in ['contains', 'startswith', 'endswith',
                          'icontains', 'istartswith', 'iendswith',
                          'isnull', 'in', 'exact', 'iexact', 'regex',
                          'iregex', 'lt', 'lte', 'gt', 'gte', 'equals',
                          'iequals', 'range']:
